...
class UserController extends Controller
{
    public function cadastrar(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->to(route('login'))->with('success', 'Sua conta foi cadastrada!');

            // Controller
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->back()->withErrors(['error' => 'Algo deu errado: ' . $e->getMessage()]);
        }
    ...
    public function logar(Request $request)
    {
        ...
        if (Auth::attempt(['documento' => $credenciais['email'], 'password' => $credenciais['password']])) {
            return redirect()->route('home');
        }
