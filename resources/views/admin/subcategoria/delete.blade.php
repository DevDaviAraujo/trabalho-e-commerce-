            ...
            <div class="card-body p-4 p-md-5">
                ...
                <div class="table-responsive rounded-3 border">
                    <table class="table table-striped table-hover mb-0 align-middle">
                        ...
                        <tbody>
                            <tr>
                                <td class="px-4">{{ $subcategoria->id }}</td>
                                <td>{{ $subcategoria->descricao }}</td>
                                <td>{{ $subcategoria->categoria->descricao }}</td>
                                <td>{{ $subcategoria->created_at->format('d/m/Y H:i') }}</td>
                                <td class="px-4">{{ $subcategoria->updated_at->format('d/m/Y H:i') }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div> <div class="card-footer bg-white p-4 p-md-5 border-0 rounded-bottom-4">
                <form method="POST" action="{{ route('deletar_subcategoria') }}">
                    @csrf
